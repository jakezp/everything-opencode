...
// LSP timing constants
const LSP_STARTUP_DELAY_MS = 100;
const LSP_INIT_DELAY_MS = 300;
const LSP_FILE_OPEN_DELAY_MS = 1000;
const LSP_DIAGNOSTICS_DELAY_MS = 500;
...
export class LSPClient {
  ...
  // Memory limits
  private static readonly MAX_STDERR_BUFFER_SIZE = 100;
  private static readonly MAX_STDERR_TEXT_LENGTH = 1000;
  private static readonly MAX_DIAGNOSTICS_CACHE_SIZE = 100;
  ...
  async start(): Promise<void> {
    ...
    this.connection.onNotification(
      ...
      (params: { uri?: string; diagnostics?: Diagnostic[] }) => {
        if (params.uri) {
          // Enforce memory limit on diagnostics cache
          if (
            this.diagnosticsStore.size >= LSPClient.MAX_DIAGNOSTICS_CACHE_SIZE
          ) {
            const firstKey = this.diagnosticsStore.keys().next().value;
            if (firstKey) this.diagnosticsStore.delete(firstKey);
          }
    ...
    if (this.proc.exitCode !== null) {
      ...
      throw new Error(
        `LSP server exited immediately with code ${this.proc.exitCode}${stderrPart}`,
      );
    }
  }
    ...
    const read = async () => {
      ...
      try {
        while (true) {
          ...
          // Truncate text and enforce buffer size limit
          const truncatedText = text.substring(
            0,
            LSPClient.MAX_STDERR_TEXT_LENGTH,
          );
          this.stderrBuffer.push(truncatedText);
          if (this.stderrBuffer.length > LSPClient.MAX_STDERR_BUFFER_SIZE) {
  ...
  async initialize(): Promise<void> {
    ...
    await new Promise((r) => setTimeout(r, LSP_INIT_DELAY_MS));
  }
  ...
  async openFile(filePath: string): Promise<void> {
    ...
    await new Promise((r) => setTimeout(r, LSP_FILE_OPEN_DELAY_MS));
  }
  ...
  async diagnostics(filePath: string): Promise<{ items: Diagnostic[] }> {
    ...
    await this.openFile(absPath);
    await new Promise((r) => setTimeout(r, LSP_DIAGNOSTICS_DELAY_MS));
