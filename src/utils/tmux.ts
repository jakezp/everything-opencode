...
/**
 * Validate and sanitize a value for shell usage.
 * Only allows alphanumeric, dashes, underscores, colons, dots, slashes.
 * Throws if invalid characters are found.
 */
function sanitizeForShell(value: string, name: string): string {
  // Allow only safe characters for URLs and session IDs
  if (!/^[a-zA-Z0-9_\-:/.]+$/.test(value)) {
    throw new Error(
      `Invalid characters in ${name}: "${value}". Only alphanumeric, dashes, underscores, colons, dots, and slashes are allowed.`,
    );
  }
  return value;
}
...
export async function spawnTmuxPane(
  ...
  try {
    ...
    const safeServerUrl = sanitizeForShell(serverUrl, 'serverUrl');
    const safeSessionId = sanitizeForShell(sessionId, 'sessionId');
    ...
    // Build command as array to avoid shell interpretation issues
    const opencodeCmd = `opencode attach ${safeServerUrl} --session ${safeSessionId}`;
